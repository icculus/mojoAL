...
static obj objects[1];  /* a panning control. */
        ...
        switch (e.type) {
            ...
            case SDL_MOUSEMOTION:
                if (draggingobj != -1) {
                    ...
                    if (o->sid != 0) {
                        float panning = ((o->x / 400.0f) - 1.0f); /* it's the pan amount. */
                        ...
                        if (panning != 0.0f)
                        {
                            if (o->channels == 1)
                            {
                                ...
                                alSourcei(o->sid, AL_SOURCE_RELATIVE, AL_TRUE);
                                alSource3f(o->sid, AL_POSITION, panning, 0.0f, -sqrtf(1.0f - panning*panning));
                            }
                            else
                            {
                                alSourcef(o->sid, AL_EXT_BALANCE, panning);
                            }
                        }
                        else
                        {
                            alSourcei(o->sid, AL_SOURCE_RELATIVE, AL_FALSE);
                            alSource3f(o->sid, AL_POSITION, 0.0f, 0.0f, 0.0f);
                            alSourcef(o->sid, AL_EXT_BALANCE, 0.f);
                        }
